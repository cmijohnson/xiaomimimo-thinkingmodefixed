...
- 新增本地 `POST /v1/messages/count_tokens` 快速估算，不再让 token 预估卡住上游
- 新增上游 keep-alive 连接池，提升并行请求吞吐
...
- `MIMO_CC_PROXY_KEEPALIVE_MS`：上游连接 keep-alive 保活时长
- `MIMO_CC_PROXY_MAX_SOCKETS`：共享连接池允许的最大并发 socket 数
...
`POST /v1/messages/count_tokens` 现在由代理本地做保守估算，不再走上游。这一条对 Claude Code 并行使用时的体感帮助很大，因为 token 预估请求本来就容易频繁出现。
...
- `mode: "count_tokens_local"`：说明这次 token 估算走的是本地快速路径，不受上游耗时影响
